...
> Status: **removed, 2026-07-21.** This flow shipped as `khb catalog` / `khb catalog-merge`
> / `khb route` and has since been deleted. The mechanical extraction half survives as
> `khb extract`; the labeling and clustering half does not.
>
> Why it was cut: it chose the bundle set — the single most consequential decision in a hub
> — from 4KB snippets, before anything had been read or curated. `khb triage` now lands a
> corpus whole in the primary bundle, and bundles are carved out afterwards from real
> concept docs and their link graph (`khb recatalog` / `khb split`), where the evidence
> actually is. See `bundles/meta/notes/decisions.md`.
>
> Kept as the rationale record for a design that was built, used, and superseded.

## Context

`khb triage` (phase 0 of `ingest.md`) already solves "index a bulk corpus without knowing
...
An earlier draft made model calls a pluggable subprocess (`khb catalog --cmd "<shell cmd>"`,
...
So: `khb` stays mechanical and dependency-free, and the brain is a **documented Claude subagent
...
### 2. `khb catalog` stays separate from `khb triage`
...
Convenience is still cheap: `khb catalog` defaults to reading `inbox/manifest.jsonl`, so
back-to-back is just `khb triage <dir> && khb catalog`.
...
becomes a cache-fill; `khb ingest`'s extractable branches (which today just log "pending
...
```
khb triage <corpus>          →  inbox/manifest.jsonl          (exists today)
   ↓  prune obvious non-knowledge from the manifest
khb catalog [--batch N]      →  inbox/extracted/<sha256>.md   (extraction cache)
...
Claude fans out Agent(model: "haiku") — one call per batch file
   ...
   ↓
khb catalog-merge            →  inbox/catalog.jsonl           (canonical, deduped)
   mechanical only
   ↓
main thread crunches facets → bundle set → khb new-bundle → inbox/routing.yaml
khb route → khb ingest <bundle>                                (exists today)
```

## Components

### `khb catalog` — mechanical, never contacts a model

`khb catalog [--batch N] [--force]`

1. Require `inbox/manifest.jsonl` (missing-file error in the style of `route.ts:12-16`,
   pointing back at `khb triage`).
...
proposes it to the user, runs `khb new-bundle` per approved category, and writes
...
- `skills/catalog/SKILL.md` — same front-matter shape as `skills/ingest/SKILL.md`. Documents
  the whole orchestration: run `khb catalog`, seed wave, hold the vocabulary, bulk wave with
  vocabulary injected, `khb catalog-merge`, retry missing batches, crunch.
...
- `ingest.md` — insert the catalog step into Phase 0 between triage and clustering, and note
  that `khb ingest` now transparently reuses the extraction cache.
...
Tesseract binary. khb is tooling, so it should carry that weight rather than push it onto
the user.

- **PDF/DOCX/ODT are now extracted by khb itself** — `unpdf` (MIT), `mammoth` (BSD-2),
...
- **OCR is opt-in**, behind `khb catalog --ocr`: `@hyzyla/pdfium` + `sharp` +
...
- **`mupdf` was rejected on licensing.** It is the obvious single package for both text and
  rendering, and it is AGPL-3.0-or-later; khb is MIT and published to npm. pdfium is BSD,
  pdf.js and tesseract.js are Apache-2.0.

## Verification

1. Small mixed scratch corpus (a few `.md`/`.txt` plus a `.pdf`, with `pdftotext` on PATH).
2. `khb triage <dir>` → `inbox/manifest.jsonl` as today.
3. `khb catalog --batch 2` → inspect `inbox/catalog/in/*.jsonl`: correct batch sizing, and the
   PDF's snippet carries real extracted text rather than an empty head.
4. Hand-write a fake `inbox/catalog/out/0001.jsonl` and run `khb catalog-merge` — exercises
...
6. Hand-write `inbox/routing.yaml`, `khb new-bundle`, `khb route`, `khb ingest <bundle>` —
...
subagent wrote its own `out/0001.jsonl` exactly to contract, and `khb ingest` populated
...
- **`khb catalog` refuses to clobber unmerged labels.** Batch files are a per-run pairing —
  `in/NNNN.jsonl` is answered by `out/NNNN.jsonl` — so a re-run wipes both. If `out/` holds
  files that were never merged, catalog errors and points at `khb catalog-merge`;
  `--reset` discards them deliberately. `catalog.jsonl` is the only durable home for labels.
- **`khb catalog-merge` clears `out/` on success** (`--keep` opts out), so the next catalog

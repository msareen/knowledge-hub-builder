...
const COMMANDS: Record<string, { load: () => Promise<unknown>; help: string }> = {
  ...
  triage: { load: () => import("./triage"), help: "khb triage <path...>            index a corpus in place (copies nothing)" },
  catalog: { load: () => import("./catalog"), help: "khb catalog [--batch N]         extract text → label batches for a model" },
  "catalog-merge": { load: () => import("./catalog-merge"), help: "khb catalog-merge               fold labeled batches → inbox/catalog.jsonl" },
  route: { load: () => import("./route"), help: "khb route                       apply inbox/routing.yaml → sources.yaml" },

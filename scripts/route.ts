...
// acquisition still happens per-bundle via `bun run ingest <bundle>`.
...
if (missing.length) {
  ...
  console.error(`Create them first: ${missing.map((b) => `bun run new-bundle ${b} "<scope>"`).join(" && ")}`);
  process.exit(1);
}
...
console.log(`\nNext: ${Object.keys(routes).map((b) => `bun run ingest ${b}`).join(" && ")}`);
